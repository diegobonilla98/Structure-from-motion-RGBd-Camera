...
def rgbd_dt(prev_rgb, prev_d, curr_rgb, curr_d):
    Rt = np.eye(4, dtype=np.float64)
    ok = odom.compute(prev_rgb, prev_d, None, curr_rgb, curr_d, None, Rt)
    return ok, Rt
...
odom = cv2.rgbd.RgbdOdometry_create(cameraMatrix=K, minDepth=MIN_DEPTH, maxDepth=MAX_DEPTH, iterCounts=(10,7,3))
...
while True:
    ...
    if prev_rgbd is not None and not stopped:
        prev_rgb = np.asarray(prev_rgbd.color)
        prev_d = np.asarray(prev_rgbd.depth)
        curr_rgb = np.asarray(rgbd.color)
        curr_d = np.asarray(rgbd.depth)
        # Convert color images to grayscale as required by odom.compute
        prev_gray = cv2.cvtColor(prev_rgb, cv2.COLOR_RGB2GRAY)
        curr_gray = cv2.cvtColor(curr_rgb, cv2.COLOR_RGB2GRAY)
        success, dT = rgbd_dt(prev_gray, prev_d, curr_gray, curr_d)
        if success:
            ...
            temp_pcd = temp_pcd.voxel_down_sample(voxel_size=0.05)  # 0.1
